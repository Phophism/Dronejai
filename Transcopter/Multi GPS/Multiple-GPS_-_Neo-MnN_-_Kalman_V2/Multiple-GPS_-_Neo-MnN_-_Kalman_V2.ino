...
String inputMessage[2];        // A string to hold incoming data
boolean IsMessageReady = false;  // Whether the string is complete
String nmea[2][15]; // GPS Data Set 1st[] -> device ID , 2nd[]Dataset
...
static const unsigned long loopInterval = 1000; // refresh every 1 second
static unsigned long lastRefreshTime = 0;

// ----------------------- Kalman variables ALT ---------------------//

float R_alt[3] = {8.96E-01,3.70E+00,4.654e-1}, Q_alt[3] = {4.654e-4,4.654e-4,4.654e-4};  //Q = process noise covariance, R = measurement noise covariance
...
float R_lat[3] = {8.41e-12,8.41e-12,8.41e-12}, Q_lat[3] = {8.5e-14,8.5e-14,8.5e-14}; //Q = process noise covariance, R = measurement noise covariance
...
float R_lng[3] = {2.209e-11,2.209e-11,2.209e-1}, Q_lng[3] = {2.18e-13,2.18e-13,2.18e-13} ;  //Q = process noise covariance, R = measurement noise covariance
...
// ----------- Read GPS 2 ------------- //  
    ...
    nmea[0][1] = ConvertLat1();
    nmea[0][3] = ConvertLng1();
    
    nmea[1][1] = ConvertLat2();
    nmea[1][3] = ConvertLng2();
...
//  Serial.print(resultLong,6);
    ...
    Serial.print(resultLat[0],6);
  //  Serial.print(",");
  //  Serial.print(Xe0_lat, 6);
    Serial.print(",");
    Serial.print(resultLng[0],6);
  //  Serial.print(",");
  //  Serial.print(Xe0_lng, 6);
    Serial.print(",");
    Serial.print(resultAlt[0],1);
    Serial.print(",");
  //  Serial.println(Xe0_alt, 4);
    Serial.print(" XXXXX ,");
    Serial.print(resultLat[1],6);
  //  Serial.print(",");
  //  Serial.print(Xe0_lat, 6);
    Serial.print(",");
    Serial.print(resultLng[1],6);
  //  Serial.print(",");
  //  Serial.print(Xe0_lng, 6);
    Serial.print(",");
    Serial.println(resultAlt[1],1);
  //  Serial.print(",");
  //  Serial.println(Xe0_alt, 4);
...
String ConvertLat1() {
  ...
  String latfirst;
  float latsecond;
  for (int i = 0; i < nmea[0][1].length(); i++) {
    if (nmea[0][1].substring(i, i + 1) == ".") {
      latfirst = nmea[0][1].substring(0, i - 2);
      latsecond = nmea[0][1].substring(i - 2).toFloat();
    }
  }
...
String ConvertLng1() {
  ...
  String lngfirst;
  float lngsecond;
  for (int i = 0; i < nmea[0][3].length(); i++) {
    if (nmea[0][3].substring(i, i + 1) == ".") {
      lngfirst = nmea[0][3].substring(0, i - 2);
      //Serial.println(lngfirst);
      lngsecond = nmea[0][3].substring(i - 2).toFloat();
